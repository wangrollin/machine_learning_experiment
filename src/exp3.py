...
if __name__ == "__main__":
    ...
    final_re2 = 0
    for i in range(5):
        test_data = all_data[i*30: (i+1)*30]
        train_data = all_data[0: i*30] + all_data[(i+1)*30: 150]
        tree = create_tree_node(train_data, [0, 1, 2, 3], 0)
        res1 = judge_all(tree, train_data)
        res2 = judge_all(tree, test_data)
        final_re1 += res1
        final_re2 += res2
        print("No." + str(i) + ": train set: " + str(res1) + ", test set: " + str(res2))
    print()
    print("final: " + "train set: " + str(final_re1/5) + ", test set: " + str(final_re2/5))
    print()
    print("hello world")
